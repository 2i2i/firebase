...
// firebase deploy --only functions:testCustomToken
...
const { Algodv2, Indexer } = require("algosdk");
// algosdk.algosToMicroalgos(1);

// const admin = require("firebase-admin");
const { initializeApp, cert } = require("firebase-admin/app");
const serviceAccount = require('./i2i-test-a79dd0357a88.json');
const app = initializeApp({
  credential: cert(serviceAccount),
  // databaseURL: 'https://yourapplication.firebaseio.com/',
  serviceAccountId: "customtokens@i2i-test.iam.gserviceaccount.com",
});
// admin.initializeApp();
...
const algorandAlgod = new Algodv2(
    "",
    process.env.ALGORAND_ALGOD,
    "",
);
const algorandIndexer = new Indexer(
    "",
...
// testCustomToken({})
exports.testCustomToken = runWith(runWithObj).https.onCall(async (data, context) => {
...
exports.cancelBid = runWith(runWithObj).https.onCall(async (data, context) => {
...
exports.ratingAdded = runWith(runWithObj).firestore
...
//       });
// };
exports.meetingCreated = runWith(runWithObj).firestore
...
exports.meetingUpdated = runWith(runWithObj).firestore.document("meetings/{meetingId}").onUpdate(async (change, context) => {
...
exports.checkUserStatus = runWith(runWithObj).pubsub.schedule("* * * * *").onRun((context) => {
  const T = new Date();
